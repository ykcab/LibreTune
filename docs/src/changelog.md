...
- **AI Assistant (enhanced)** — The assistant now *applies* approved proposals to the working tune (restore point first, still never burns); enforces the capability tier (read / tune / config, cumulative) on both the offered tools and incoming calls; reads live sensor snapshots and datalogs (summary stats or recent rows); flags pin conflicts and large batch shifts on proposals; shows live "reading X…" progress, a per-chat token counter, and an **Ask AI** button on every table editor; stores the API key in the OS keychain; and offers provider presets including local Ollama / LM Studio. Read results follow your display-unit preferences.
